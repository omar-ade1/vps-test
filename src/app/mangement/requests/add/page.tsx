...
import { ACCEPT_OR_REJECT_USER_TO_COURSE } from "@/app/fetchApi/requestCourse/acceptOrRejectUser";
...
const AddRequests = () => {
  ...
  const alertConfirm = async (id: number, courseId: number, status: string, message: string) => {
    ...
    if (confirm) {
      handelUpdate(id, courseId, status);
    }
  };
   ...
   */
  const handelUpdate = async (id: number, courseId: number, status: string): Promise<void> => {
    const message: any = await ACCEPT_OR_REJECT_USER_TO_COURSE(id, courseId, status);
  ...
  return (
    <main className="min-h-[calc(100vh-100px)] pt-[50px] pb-[100px] ">
      <TitleForPage titleText="اضافة مشتركين" />
                  ...
                  <div className="my-5 border-2 p-5 shadow-xl" key={course.id}>
                    ...
                    <Table className="shadow-xl p-5" key={course.id}>
                      <TableHeader>
                        ...
                        <TableColumn>قبول</TableColumn>
                        <TableColumn>رفض</TableColumn>
                      </TableHeader>

                      <TableBody emptyContent={"لا توجد طلبات انضمام لهذا الكورس"}>
                        {course.EnrollmentRequest
                          ? course.EnrollmentRequest.filter((e) => e.status === "Pending").map((e) => (
                              <TableRow key={e.id}>
                                <TableCell>{e.user.userName}</TableCell>
                                <TableCell>{format(new Date(e.createdAt), "Pp")}</TableCell>
                                <TableCell>
                                  <Button
                                    onClick={() => {
                                      alertConfirm(
                                        ...
                                        course.id,
                                        "Accepted",
                                        `هل تريد تأكيد اضافة هذا الطالب (${e.user.userName}) لهذا الكورس (${course.courseName})`
                                      );
                                    }}
                                    color="success"
                                  >
                                  ...
                                  <Button
                                    onClick={() => {
                                      alertConfirm(
                                        ...
                                        course.id,
                                        "Rejected",
                                        `هل تريد تأكيد رفض انضمام هذا الطالب (${e.user.userName}) لهذا الكورس (${course.courseName})`
                                      );
                                    }}
                                    color="danger"
                                  >
